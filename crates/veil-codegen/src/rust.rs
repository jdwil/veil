...
pub(crate) struct ModuleContents<'a> {
...
fn is_harness_config_construct(c: &Construct, registry: &LayerRegistry) -> bool {
...
pub(crate) fn flatten_module<'a>(module: &'a Construct, registry: &LayerRegistry) -> ModuleContents<'a> {
...
pub(crate) fn http_routable_services<'a>(
...
pub(crate) fn build_name_to_shape(solution: &Solution, registry: &LayerRegistry) -> std::collections::HashMap<String, Shape> {
    use std::collections::HashMap;
...
fn gen_application(flows: &[FlowLike], module_contents: &ModuleContents, crate_name: &str, solution: &Solution, registry: &LayerRegistry, deps_decl: Option<&veil_ir::DepsDecl>) -> GeneratedFile {
...
pub(crate) fn collect_deps_field_map(
    fns: &[&Construct],
    registry: &LayerRegistry,
    name_to_shape: &std::collections::HashMap<String, Shape>,
) -> (
