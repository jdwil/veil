...
#[test]
fn lower_range_to_array_lit() {
    // Range now emits an ArrayLit with start and end elements
    let expr = Expr::Range {
    ...
    match &ts {
        TsExpr::ArrayLit { items, .. } => {
            ...
            assert_eq!(emit_ts(&items[0]), "0");
            assert_eq!(emit_ts(&items[1]), "10");
        }
        other => panic!("Expected ArrayLit for Range, got {:?}", other),
    }
}
...
#[test]
fn lower_range_to_array() {
    // Range has no native TS equivalent → emits as [start, end] array
    let expr = Expr::Range {
        start: Some(Box::new(Expr::IntLit(0))),
        end: Some(Box::new(Expr::IntLit(10))),
        inclusive: false,
...
fn ctx_with_routing(trait_name: &str) -> GenCtx {
    ...
    // Simulate bus.layer: register method_lowers_to for invoke/dispatch/request
    let mut invoke_targets = std::collections::HashMap::new();
    invoke_targets.insert("typescript".to_string(), "await deps.{dep}.invoke({args})".to_string());
    let mut dispatch_targets = std::collections::HashMap::new();
    dispatch_targets.insert("typescript".to_string(), "await deps.{dep}.dispatch({args})".to_string());
    ctx.method_lowers_to.insert((trait_name.to_string(), "invoke".to_string()), invoke_targets);
    ctx.method_lowers_to.insert((trait_name.to_string(), "dispatch".to_string()), dispatch_targets);
    ctx
}
...
fn lower_routing_invoke_with_struct_lit() {
    ...
    assert_eq!(
        output,
        "await deps.bus.invoke({ orderId, amount: 500 })"
    );
}
...
fn lower_routing_dispatch_plain() {
    ...
    let ts = lower_to_ts(&Expr::Call(call), &ctx);
    let output = emit_ts(&ts);
    assert_eq!(output, "await deps.event_bus.dispatch(event)");
}
