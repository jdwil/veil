...
async function viewDashboard() {
  ...
  try {
    ...
    projects = data.repos || data.projects || [];
  } catch (e) {
    // Empty state is fine — show dashboard with 0 projects
    projects = [];
  }
  shell(el("div", {},
    el("h1", {}, "Dashboard"),
    el("p", { className: "sub" }, "VEIL Runtime Control Plane"),
    ...
    }) : [el("p", { className: "sub" }, "No projects yet. Create one to get started.")]),
  ));
}
...
async function viewPage(title, load) {
  try {
    ...
    shell(el("div", {}, el("h1", {}, title), el("p", { className: "sub" }, "No data yet.")));
  }
}
...
async function viewConfig() {
  ...
  try { cfg = await api("/api/config"); } catch (e) {
    cfg = { projects_dir: "~/.veil/projects", config_path: "(not yet configured)" };
  }
